...
class GeneratorsTable extends \WooCommerceSerialNumbers\Admin\ListTables\ListTable {
	...
	protected function column_validity_for( $item ) {
		...
		if ( ! empty( $item->validity_for ) ) {
			...
			return sprintf( _nx( '%d day <small>After Purchase</small>', '%d days <small>After Purchase</small>', $item->validity_for, 'valid for days', 'wc-serial-numbers' ), $item->validity_for );
		}
