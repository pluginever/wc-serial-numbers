...
class Requests {
	...
	public function edit_generator() {
		...
		WCSN()->add_notice( __( 'Generator has been saved successfully.', 'wc-serial-numbers' ) );
	...
	public static function generate_bulk_keys() {
		...
		if ( function_exists( 'wcsn_get_manager_role' ) && ! current_user_can( wcsn_get_manager_role() ) ) {
			wp_send_json_error( array( 'message' => __( 'You are not allowed, to use this.', 'wc-serial-numbers' ) ) );
		}
		...
		if ( empty( $product_id ) ) {
			WCSN()->add_notice( __( 'Error: Please select a product.', 'wc-serial-numbers' ), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		...
		if ( empty( $quantity ) ) {
			WCSN()->add_notice( __( 'Error: Please enter a quantity.', 'wc-serial-numbers' ), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		...
		if ( empty( $keys ) ) {
			WCSN()->add_notice( __( 'Could not generate any keys. Please check the generator settings.', 'wc-serial-numbers' ), 'error' );
			wp_safe_redirect( $referer );
			exit;
		}
		...
		$notice = sprintf( esc_html__( '%s keys have been generated successfully.', 'wc-serial-numbers' ), number_format( count( $keys ) ) );
