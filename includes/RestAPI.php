...
class RestAPI {
	...
	public function validate_key( $request ) {
		...
		$response = array(
			...
			'activations'      => $this->prepare_activations( $serial_key ),
	...
	/**
	 * Prepare the activations for the response.
	 *
	 * @since 1.7.3
	 * @param \WooCommerceSerialNumbers\Models\Key $serial_key Serial key model.
	 * @return array List of activation data.
	 */
	protected function prepare_activations( $serial_key ) {
		return array_map(
			function ( $activation ) {
				return $activation->to_array();
			},
			(array) $serial_key->activations
		);
	}
}
