...
function wcsn_edit_serial_number( $data ) {
	...
	$serial_id        = wcsn_insert_serial_number( [
		...
		'serial_key'       => wcsn_encrypt_serial_number( $serial_key ),
	...
	if ( is_wp_error( $serial_id ) ) {
		wcsn_add_admin_notice( $serial_id->get_error_message(), 'error' );
	...
	$message = '';
	if ( $id == $serial_id ) {
		$message = __( 'Serial Number has updated successfully', 'wc-serial-numbers' );
	} else {
		$message = __( 'Serial Number has inserted successfully', 'wc-serial-numbers' );
	}
	wcsn_add_admin_notice( $message );
...
function wcsn_order_assign_serial_number_handler() {
	...
	$assigned = wcsn_order_assign_serial_numbers( $order_id );
	if ( $assigned ) {
		wcsn_add_admin_notice( __( 'Serial numbers has been assigned for the order.', 'wc-serial-number' ) );
	}
...
add_action( 'wcsn_admin_get_order_assign_serial_numbers', 'wcsn_order_assign_serial_number_handler' );
 ...
 * @param $data
 */
function wcsn_inactive_serial_number_handler( $data ) {
...
add_action( 'wcsn_admin_get_inactive_serial_number', 'wcsn_inactive_serial_number_handler' );
 ...
 * @param $data
 */
function wcsn_activate_serial_number_handler( $data ) {
	if ( ! wp_verify_nonce( $data['_wpnonce'], 'serial_number_nonce' ) ) {
		wp_die( 'cheatin??' );
	}
	...
	wcsn_change_serial_number_status( $id, 'active' );
	wp_redirect( admin_url( 'admin.php?page=wc-serial-numbers' ) );
	exit();
}

add_action( 'wcsn_admin_get_activate_serial_number', 'wcsn_activate_serial_number_handler' );

/**
 * @since 1.0.0
 * @param $data
 */
function wcsn_delete_serial_number_handler( $data ) {
	if ( ! wp_verify_nonce( $data['_wpnonce'], 'serial_number_nonce' ) ) {
		wp_die( 'cheatin??' );
	}
...
add_action( 'wcsn_admin_get_delete_serial_number', 'wcsn_delete_serial_number_handler' );
 ...
 * @param $data
 */
function wcsn_activate_activation_handler( $data ) {
...
add_action( 'wcsn_admin_get_activate_activation', 'wcsn_activate_activation_handler' );
 ...
 * @param $data
 */
function wcsn_inactivate_activation_handler( $data ) {
	if ( ! wp_verify_nonce( $data['_wpnonce'], 'wcsn_activation_nonce' ) ) {
		wp_die( 'cheatin??' );
	}
...
add_action( 'wcsn_admin_get_inactivate_activation', 'wcsn_inactivate_activation_handler' );

function wcsn_delete_activation_handler($data){
	if ( ! wp_verify_nonce( $data['_wpnonce'], 'wcsn_activation_nonce' ) ) {
		wp_die( 'cheatin??' );
	}
...
add_action( 'wcsn_admin_get_delete_activation', 'wcsn_delete_activation_handler' );