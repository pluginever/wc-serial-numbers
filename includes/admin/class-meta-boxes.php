...
class Meta_Boxes extends Controller {
	...
	public static function variable_product_content( $loop, $variation_data, $variation ) {
		if ( ! wc_serial_numbers()->is_pro_active() ) {
			echo sprintf(
				...
				esc_html__( 'WooCommerce Serial Number Free version does not support product variation.', 'wc-serial-numbers' ),
	...
	/**
	 * Render order item meta.
	 *
	 * @param int            $o_item_id Order item id.
	 * @param \WC_Order_Item $o_item Order item.
	...
	public function order_itemmeta( $o_item_id, $o_item, $product ) {
		...
		if ( empty( $items ) && $order ) {
			echo sprintf( '<div class="wcsn-missing-serial-number">%s</div>', esc_html__( 'Order missing serial numbers for this item.', 'wc-serial-numbers' ) );
