...
class WC_Serial_Numbers_Activations_List_Table extends \WP_List_Table {
	...
	function column_instance( $item ) {
		$actions           = array();
		$base_url          = add_query_arg( array( 'activation' => $item->id ), admin_url( 'admin.php?page=wc-serial-numbers-activations' ) );
		$activate_url      = wp_nonce_url( add_query_arg( [ 'serial_numbers_action' => 'activate_activation' ], $base_url ), 'wcsn_activation_nonce' );
		$deactivate_url    = wp_nonce_url( add_query_arg( [ 'serial_numbers_action' => 'inactivate_activation' ], $base_url ), 'wcsn_activation_nonce' );
		$delete_url        = wp_nonce_url( add_query_arg( [ 'serial_numbers_action' => 'delete_activation' ], $base_url ), 'wcsn_activation_nonce' );
