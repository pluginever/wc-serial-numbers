...
class Serial_Numbers_Table extends \WP_List_Table {
	/**
	...
	function column_key( $item ) {
		$row_actions         = array();
		$base_url            = add_query_arg( array( 'id' => $item->id ), admin_url( 'admin.php?page=serial-numbers' ) );
		...
		$row_actions['show'] = sprintf( '<a data-serial-id="%d" data-nonce="%s" class="serial-decrypt-key"   href="#">%s</a>', $item->id, wp_create_nonce( 'wc_serial_numbers_decrypt_key' ), __( 'Show', 'wc-serial-numbers' ) );
		...
		if ( ! wc_serial_numbers()->get_settings('hide_serial_number', false, true) ) {
			$class      = 'decrypted';
			$serial_key = apply_filters( 'wc_serial_numbers_maybe_decrypt', $item->serial_key );
			unset( $row_actions['show'] );
		}

		return sprintf( '<code class="serial-key %1$s">%2$s</code> %3$s%4$s', $class, $serial_key, $spinner, $this->row_actions( $row_actions ) );
	}
	...
	function column_product( $item ) {
		...
		return empty( $item->product_id ) || empty( $product ) ? '&mdash;' : sprintf( '<a href="%s" target="_blank">%s</a>', get_edit_post_link( $post_id ),  Helper::get_product_title( $product ) );
	}
	...
	function column_status( $item ) {
		return sprintf( "<span class='serial-key-status %s'>%s</span>", sanitize_html_class( $item->status ), Helper::get_serial_status( $item, 'view' ) );
	}
	...
	public function get_results() {
		...
		$query = Query_Serials::init( 'serials_list_table' )
		...
		if ( ! empty( $search ) ) {
			$query->orWhere( 'serial_key', 'LIKE', '%' . Helper::encrypt( $search ) . '%' );
