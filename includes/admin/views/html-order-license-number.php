	...
	<div class="wcsn-metabox-table-wrapper">
		<table class="wcsn-metabox-table wp-list-table widefat fixed striped" cellspacing="0">
			...
			<tr>
				...
				<th><?php _e( 'Validity', 'wc-serial-numbers' ) ?></th>
			...
			foreach ( $serial_numbers as $serial_number ): $i ++ ?>
				<tr class="<?php echo ( $i % 2 == 1 ) ? 'alternate' : ''; ?>">
					<td><a href="<?php echo get_edit_post_link( $serial_number->product_id ); ?>"><?php echo get_the_title( $serial_number->product_id ); ?></a></td>
					...
					<td><?php echo ! empty( $serial_number->validity ) ? sprintf( _n( '%s Day', '%s Days', $serial_number->validity, 'wc-serial-numbers' ), number_format_i18n( $serial_number->validity ) ) : __( 'Never expire', 'wc-serial-numbers' ); ?></td>
