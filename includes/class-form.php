...
class WC_Serial_Numbers_Form{
	...
	public static function input_control( $args ) {
		...
		$html = sprintf( '<div class="p-ever-form-group %s">', implode( ' ', $wrapper_classes ) );
		$html .= ! empty( $label ) ? sprintf( '<label for="%1$s" class="p-ever-control-label">%2$s</label>', $id, $label ) : '';
		$html .= $button || $icon ? '<div class="p-ever-input-group">' : '';
		$html .= sprintf( '<div class="p-ever-input-group-addon">%s</div>', $icon );
		$html .= sprintf( '<input type="%1$s" class="p-ever-form-control %2$s-text %7$s" id="%3$s" name="%4$s" value="%5$s" %6$s autocomplete="off"/>', $type, $size, $id, $name, $value, $attributes, $input_classes );
		$html .= $button ? $html .= sprintf( '<div class="p-ever-input-group-btn">%s</div>', $button ) : '';
		$html .= $button || $icon ? '</div><!--.p-ever-input-group-->' : '';
		$html .= $description ? $description : '';
		$html .= '</div><!--.p-ever-form-group-->';
	...
	public static function select_control( $args ) {
		...
		$input_classes     = is_array( $args['class'] ) ? $args['class'] : explode( ' ', $args['class'] );
		$input_classes[]   = ( true == $args['select2'] ) ? 'p-ever-select2-control' : '';
		$wrapper_classes   = is_array( $args['wrapper_class'] ) ? $args['wrapper_class'] : explode( ' ', $args['wrapper_class'] );
		$wrapper_classes[] = ( true == $args['required'] ) ? 'required' : '';
		$icon              = empty( $args['icon'] ) ? false : sprintf( '<i class="%s"></i>', $args['icon'] );
		$button            = empty( $args['button'] ) ? false : self::sanitize_button( $args['button'] );
		...
		$html .= $button || $icon ? '<div class="p-ever-input-group">' : '';
		$html .= sprintf( '<div class="p-ever-input-group-addon">%s</div>', $icon );
		$html .= sprintf( '<select class="p-ever-form-control %1$s" name="%2$s" id="%3$s" %4$s>', $input_classes, $name, $id, $attributes );
	...
	public static function textarea_control( $args ) {
		...
		$html .= $button || $icon ? '<div class="p-ever-input-group">' : '';
		$html .= sprintf( '<div class="p-ever-input-group-addon">%s</div>', $icon );
		$html .= sprintf( '<textarea rows="%7$s" cols="%8$s" class="p-ever-form-control %1$s-text %6$s" id="%2$s" name="%3$s" %5$s>%4$s</textarea>', $size, $id, $name, $value, $attributes, $input_classes, $rows, $cols );
		$html .= $button ? $html .= sprintf( '<div class="p-ever-input-group-btn">%s</div>', $button ) : '';
		$html .= $button || $icon ? '</div><!--.p-ever-input-group-->' : '';
		$html .= $description ? $description : '';
		$html .= '</div><!--.p-ever-form-group-->';
	...
	public static function switch_control( $args ) {
		...
		$html = sprintf( '<div class="p-ever-form-group p-ever-switch %s">', implode( ' ', $wrapper_classes ) );
		$html .= ! empty( $label ) ? sprintf( '<label for="%1$s" class="p-ever-control-label">%2$s</label>', $id, $label ) : '';
		$html .= sprintf( '
				<fieldset>
					<label for="%1$s">
					<input type="checkbox" class="%2$s" id="%3$s" name="%4$s" value="%5$s" %6$s%7$s/>
					<span class="p-ever-switch-view"></span>
					</label>
	...
	public static function checkboxes_control( $args ) {
		...
		$html = sprintf( '<div class="p-ever-form-group p-ever-checkbox-group %s">', implode( ' ', $wrapper_classes ) );
		$html .= ! empty( $label ) ? sprintf( '<label for="%1$s" class="p-ever-control-label">%2$s</label>', $id, $label ) : '';
		$html .= '<fieldset class="p-ever-checkboxes">';
		foreach ( $args['options'] as $key => $label ) {
			$checked = in_array( $key, $value ) ? $key : '0';
			$html    .= sprintf( '<div class="p-ever-check-input">' );
			$html    .= sprintf( '<label for="%1$s[%2$s]">', $id, $key );
			$html    .= sprintf( '<input type="checkbox" class="checkbox p-ever-check-control" id="%1$s[%3$s]" name="%2$s[]" value="%3$s" %4$s />', $id, $name, $key, checked( $checked, $key, false ) );
