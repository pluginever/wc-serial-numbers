...
function wc_serial_numbers_get_order_table( $order, $return = false ) {
	...
	$serial_numbers = Query::init()->from( 'serial_numbers' )->where( 'order_id', intval( $order_id ) )->get();
...
function wc_serial_numbers_order_connect_serial_numbers( $order_id ) {
	...
	foreach ( $items as $item ) {
		...
		$delivered_qty                  = Query::init()->table( 'serial_numbers' )->where( 'order_id', $order_id )->where( 'product_id', $product_id )->count();
		...
		$serials = Query::init()->table( 'serial_numbers' )
...
function wc_serial_numbers_order_disconnect_serial_numbers( $order_id ) {
	...
	if ( $reuse_serial_number ) {
		global $wpdb;
		Query::init()->table( 'serial_numbers' )->whereRaw( $wpdb->prepare( "serial_id IN (SELECT id from {$wpdb->prefix}serial_numbers WHERE order_id=%d)", $order_id ) )->delete();
	}
	do_action( 'wc_serial_numbers_pre_order_disconnect_serial_numbers', $order_id );

	$total_disconnected = Query::init()->table( 'serial_numbers' )->where( 'order_id', $order_id )->update( $data );
...
function wc_serial_numbers_get_serial_number( $id ) {
	return Query::init()->table( 'serial_numbers' )->find( intval( $id ) );
}
...
function wc_serial_numbers_get_activation( $activation_id ) {
	return Query::init()->from( 'serial_numbers_activations' )->find( intval( $activation_id ) );
}
...
function wc_serial_numbers_update_activation_count( $id ) {
	$activation = Query::init()->from( 'serial_numbers_activations' )->find( $id );
	if ( empty( $activation ) ) {
		return false;
	}
	global $wpdb;
...
function wc_serial_numbers_get_stock_quantity( $product_id ) {
	...
	if ( 'custom_source' === get_post_meta( $product_id, '_serial_key_source', true ) || empty( $source ) ) {
		return Query::init()->from( 'serial_numbers' )->where( [
