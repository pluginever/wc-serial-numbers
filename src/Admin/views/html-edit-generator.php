...
<div class="wrap pev-wrap">
	<h1 class="wp-heading-inline">
		<?php if ( $generator->exists() ) : ?>
			<?php esc_html_e( 'Edit generator', 'wc-serial-numbers-pro' ); ?>
		<?php else : ?>
			<?php esc_html_e( 'Add New Generator', 'wc-serial-numbers-pro' ); ?>
		<?php endif; ?>
		<a href="<?php echo esc_attr( admin_url( 'admin.php?page=wc-serial-numbers-generators' ) ); ?>" class="page-title-action">
			<?php esc_html_e( 'Back', 'wc-serial-numbers-pro' ); ?>
		</a>
	</h1>
	<form method="post" action="<?php echo esc_html( admin_url( 'admin-post.php' ) ); ?>">
		<table class="form-table">
			...
			<tr>
				<th scope="row">
					<label for="name">
						<?php esc_html_e( 'Name', 'wc-serial-numbers-pro' ); ?>
				...
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Enter a friendly name for the generator.', 'wc-serial-numbers-pro' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				...
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Enter a pattern for this generator. Use # for random characters and y, m and d for year, month and date respectively.', 'wc-serial-numbers-pro' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="charset">
						<?php esc_html_e( 'Charset', 'wc-serial-numbers-pro' ); ?>
						<abbr title="required">*</abbr>
					</label>
				</th>
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Enter the charset for the generator. Leave empty for default charset.', 'wc-serial-numbers-pro' ); ?>
					</p>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
					<label for="valid_for">
						<?php esc_html_e( 'Valid for (days)', 'wc-serial-numbers-pro' ); ?>
					</label>
				</th>
				<td>
					<input name="valid_for" id="valid_for" class="regular-text" type="number" value="<?php echo esc_attr( $generator->valid_for ); ?>">
					<p class="description">
						<?php esc_html_e( 'Number of days the key will be valid from the purchase date. Leave it blank for lifetime validity.', 'wc-serial-numbers-pro' ); ?>
					</p>
				</td>
			</tr>
				...
				<tr valign="top">
					<th scope="row">
						<label for="activation_limit">
							...
							<?php echo wp_kses_post( wc_help_tip( esc_html__( 'Maximum number of times the key can be used to activate the software. If the product is not software, keep it blank.', 'wc-serial-numbers-pro' ), true ) ); ?>
						</label>
					</th>
					<td>
						...
						<p class="description">
							<?php esc_html_e( 'Maximum number of times the key can be used to activate the software. If the product is not software, keep it blank.', 'wc-serial-numbers-pro' ); ?>
						</p>
					</td>
				</tr>
			<?php endif; ?>
			<tr>
				...
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Select the status for this generator.', 'wc-serial-numbers-pro' ); ?>
					</p>
				</td>
			</tr>
			</tbody>
			...
			<tr>
				<td colspan="2">
					...
					<?php submit_button( $generator->exists() ? esc_html__( 'Save Changes', 'wc-serial-numbers-pro' ) : esc_html__( 'Create', 'wc-serial-numbers-pro' ), 'primary' ); ?>
				</td>
			</tr>
			</tfoot>
		</table>
