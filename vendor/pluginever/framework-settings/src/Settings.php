...
abstract class Settings
{
     ...
     *
     * @since 1.0.0
     * @deprecated 1.0.3 Use static::instance() instead.
     * @return self
     */
    ...
    protected function output_plugins_widget()
    {
        ...
        if (!empty($promo_plugins)) {
            ...
            foreach ($promo_plugins as $promo_plugin) {
                $promo_plugin = wp_parse_args($promo_plugin, array('name' => '', 'description' => '', 'basename' => '', 'slug' => '', 'badge' => esc_html__('Recommended', 'wc-serial-numbers'), 'button' => esc_html__('Install Now', 'wc-serial-numbers'), 'installed' => false));
                ...
                if (!empty($promo_plugin['badge'])) {
                    ?>
						<span class="pev-panel__legend"><?php 
    ...
    protected function output_support_widget()
    {
        ...
        if (!empty($support_links)) {
			...
			<div class="pev-panel">
				<h3><?php 
            esc_html_e('Need Help?', 'wc-serial-numbers');
            ?></h3>
				<ul>
					<?php 
            foreach ($support_links as $support_link) {
                ?>
						<li>
							<a href="<?php 
                echo esc_url($support_link['url']);
                ?>" target="_blank">
								<?php 
                echo esc_html($support_link['label']);
                ?>
							</a>
						</li>
					<?php 
    ...
    {
        return array(array('name' => 'WC Min Max Quantities', 'slug' => 'wc-min-max-quantities', 'description' => 'Set minimum and maximum price or quantity for WooCommerce products.', 'link' => 'https://wordpress.org/plugins/wc-min-max-quantities/', 'badge' => esc_html__('Recommended', 'wc-serial-numbers'), 'button' => esc_html__('Install Now', 'wc-serial-numbers')));
    }
    /**
     * Get support links.
     *
    ...
    {
        return array('facebook' => array('label' => __('Join our Community', 'wc-serial-numbers'), 'url' => 'https://www.facebook.com/groups/pluginever'), 'feature-request' => array('label' => __('Request a Feature', 'wc-serial-numbers'), 'url' => 'https://www.pluginever.com/contact/'), 'bug-report' => array('label' => __('Report a Bug', 'wc-serial-numbers'), 'url' => 'https://www.pluginever.com/contact/'));
    }
    /**
     * Get current page.
     *
